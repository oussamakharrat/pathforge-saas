...
export function Field({ label, type = "text", placeholder = "", value, onChange, Left, Right, error, className = "" }: {
  label?: string; type?: string; placeholder?: string; value: string; onChange: (v: string) => void;
  Left?: React.ElementType; Right?: React.ReactNode; error?: string; className?: string;
}) {
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      <div className="relative">
        ...
        <input type={type} value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)}
          className={cn("w-full h-11 rounded-xl border text-[13px] bg-input-background text-foreground placeholder:text-muted-foreground transition-all focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/60", Left ? "pl-10 pr-4" : "px-4", Right ? "!pr-10" : "", error ? "border-red-400 focus:ring-red-100 focus:border-red-400" : "border-border/80")} />
